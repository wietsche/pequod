...
s3missionFile = os.environ['MISSION']
...
def workdir():
    ...
    #print cmd
    os.system(cmd)
    cmd = 'ssh -o "StrictHostKeyChecking no" -i $PEQUOD_HOME/wietsche.pem root@$PEQUOD_PUBLIC_IP "'+cmd+'"'
    #print cmd
    os.system(cmd)
    return
...
cmd = 'scp -o "StrictHostKeyChecking no"  -i $PEQUOD_HOME/wietsche.pem /tmp/'+ signature + ' root@$PEQUOD_PUBLIC_IP:/tmp/'+signature
os.system(cmd)
...
for key in keys:
    ...
    if "wav" in infile: 
        ...
        if not (filetime in procd):
            ...
            cmd = 'scp -o "StrictHostKeyChecking no" -i $PEQUOD_HOME/wietsche.pem /tmp/'+ signalfile + ' root@$PEQUOD_PUBLIC_IP:/tmp/'+signalfile
            print cmd
            os.system(cmd)
            ...
            try:
                cmd = 'scp -o "StrictHostKeyChecking no" -i $PEQUOD_HOME/wietsche.pem root@$PEQUOD_PUBLIC_IP:/tmp/'+csvfile + ' /tmp/' + csvfile
                print cmd
                os.system(cmd)
